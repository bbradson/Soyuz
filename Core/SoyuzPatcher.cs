    ...
    public class SoyuzPatchInfo
    {
        ...
        public void Patch(Harmony harmony)
        {
            ...
            foreach (var target in targets.ToHashSet())
            {
                ...
                try
                {
                    ...
                    Log.Message($"SOYUZ: Patched {target.DeclaringType.Name}:{target}");
                }
                catch (Exception er)
                {
                    Log.Warning($"SOYUZ: patching {target.DeclaringType.Name}:{target} is not possible!");
        ...
        public void Unpatch(Harmony harmony)
        {
            foreach (var target in targets.ToHashSet())
            {
                ...
                {
                    Log.Warning($"SOYUZ: Unpatching {target.DeclaringType.Name}:{target} is not possible!");
                }
            }
        }
    }
    ...
    public class SoyuzPatcher
    {
        ...
        static SoyuzPatcher()
        {
            ...
            foreach (var type in flaggedTypes)
            {
                ...
                patchList.Add(patch);
                Log.Message($"SOYUZ: found patch in {type} and is {(patch.IsValid ? "valid" : "invalid") }");
            }
